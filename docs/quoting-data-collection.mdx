...
The `DataSourceCollector` is an interface that abstracts the logic for collecting data from various sources and manages its own lifecycle and data processing. It is useful when dealing with different data sources that have varying connection methods and update frequencies. It is used with a `DataCollectionStorage` backend to which it passes the collected data.

**Interface**

The interface includes a single method: `run(storage: DataCollectionStorage)`, which is designed to be asynchronous for flexible data collection and processing. This universal method can operate in various modes—either running in an infinite loop to listen to a WebSocket or performing periodic polling via HTTP.
...
The `DataCollector` class can simply call the `run()` method for each DataSourceCollector instance. Each `DataSourceCollector` operates independently, polling its data source or listening to a WebSocket, and sends the data to the specified storage.
...
It is needed to have a function that is similar to `load_feeds()` to load `DataSourceCollector` instances from a configuration file. Probably, the config should not be extended with any additional fields. Instead, the `feeds` config should be used, because only feeds define which data should be collected. For example, it might be done by adding a method to `PriceFeed` to get needed `DataSourceCollector` instances.

### How Can It Be Used

The `DataCollector` can operate as an independent daemon, focused on accumulating required data in isolation from other processes, particularly the `Updater`. But this allows the Updater to easily utilize the aggregated data by using modified Feeds that can access the `DataCollectionStorage`.
