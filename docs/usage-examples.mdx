...
The usage of Multidata oracle in a smart contract is quite easy. The examples below show how to get prices of `ETH` and `NEAR` on Arbitrum chain. The list of supported assets in each chain could be found on the page of the corresponding network here https://multidata.ai/.

## Main concepts

### Proxies

The development of contracts is continuing and a new version of the oracle contract could be deployed. To avoid changing the oracle contract address on each such deployment consumers can use an address of a proxy contract, which has the same interface (`ICoreMultidataFeedsReader`) as the oracle contract.
...
All prices are stored with the 2**112 multiplier. It allows to use fractional values. E.g.:

▪️ 0.01 is stored as `0.01 * 2**112` = `51922968585348276285304963292200`   
▪️ 100 is stored as `100 * 2**112` = `519229685853482762853049632922009600`   

## Examples in popular languages

### Solidity

To use oracle prices in your smart contract, the `ICoreMultidataFeedsReader` interface should be used.
...
This example uses [web3.js](https://web3js.readthedocs.io/en/v1.10.0/) to fetch prices of `ETH` and `NEAR` on Arbitrum chain.
...
This example uses [ethers.js](https://docs.ethers.org/v6/) to fetch prices of `ETH` and `NEAR` on Arbitrum chain.
...
This example uses [web3.py](https://web3py.readthedocs.io/en/stable/) to fetch prices of `ETH` and `NEAR` on Arbitrum chain.
...
Multidata oracle is used in production in Unit protocol. See UnitMetadataOracle here https://github.com/unitprotocol/core/blob/main/CONTRACTS.md#oracles
